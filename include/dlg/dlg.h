...
// the base path stripped from __FILE__. If you don't override DLG_FILE set this
// to the project root to make 'main.c' from '/some/bullshit/main.c'
...
// The function used for formatting. Can have any signature, but must be
// callable with the arguments the log/assertions macros are called with. Must
// return a const char* that will not be freed by dlg, the formatting function
// must keep track of it. The formatting function might use dlg_thread_buffer or
// a custom owned buffer. The returned const char* has to be valid until the dlg
// log/assertion ends. Usually a c function with ... (i.e. using va_list) or a
// variadic c++ template do allow formatting.
#ifndef DLG_FMT_FUNC
...
#define DLG_CREATE_TAGS(...)                                                   \
    std::initializer_list<const char *>{DLG_DEFAULT_TAGS_TERM, __VA_ARGS__,    \
                                        NULL}                                  \
        .begin()
#else
#define DLG_CREATE_TAGS(...)                                                   \
    (const char *const[]) { DLG_DEFAULT_TAGS_TERM, __VA_ARGS__, NULL }
#endif
...
enum dlg_level {
    dlg_level_trace =
        0, // temporary used debug, e.g. to check if control reaches function
    dlg_level_debug, // general debugging, prints e.g. all major events
    dlg_level_info,  // general useful information
    dlg_level_warn,  // warning, something went wrong but might have no (really
                     // bad) side effect
...
typedef void (*dlg_handler)(const struct dlg_origin *origin, const char *string,
                            void *data);
...
//   dlg_asserttlm(("tag1), dlg_level_warning, data != nullptr, "Data must not
//   be null"); dlg_assertlm(dlg_level_error, data != nullptr, "Data must not be
//   null");
#define dlg_assertl(level, expr)                                               \
    if (level >= DLG_ASSERT_LEVEL && !(expr))                                  \
    dlg__do_log(level, DLG_CREATE_TAGS(NULL), DLG_FILE, __LINE__, __func__,    \
                NULL, DLG_FAILED_ASSERTION_TEXT(#expr))
#define dlg_assertlt(level, tags, expr)                                        \
    if (level >= DLG_ASSERT_LEVEL && !(expr))                                  \
    dlg__do_log(level, DLG_CREATE_TAGS tags, DLG_FILE, __LINE__, __func__,     \
                NULL, DLG_FAILED_ASSERTION_TEXT(#expr))
...
#define dlg_assertltm(level, tags, expr, ...)                                  \
    if (level >= DLG_ASSERT_LEVEL && !(expr))                                  \
    dlg__do_log(level, DLG_CREATE_TAGS tags, DLG_FILE, __LINE__, __func__,     \
                DLG_FMT_FUNC(__VA_ARGS__), DLG_FAILED_ASSERTION_TEXT(#expr))

#define dlg__assert_or(level, tags, expr, code, msg)                           \
    if (!(expr)) {                                                             \
...
// Formats the given format string and arguments as printf would, uses the
// thread buffer.
DLG_API const char *dlg__printf_format(const char *format, ...)
    DLG_PRINTF_ATTRIB(1, 2);
DLG_API void dlg__do_log(enum dlg_level lvl, const char *const *, const char *,
...
// It also flushes the stream used and correctly outputs even from multiple
// threads.
DLG_API void dlg_default_output(const struct dlg_origin *, const char *string,
                                void *);
...
// Adds the given tag associated with the given function to the thread specific
// list. If func is not NULL the tag will only applied to calls from the same
// function. Remove the tag again calling dlg_remove_tag (with exactly the same
// pointers!). Does not check if the tag is already present.
...
// The pointers must be exactly the same pointers that were supplied to
// dlg_add_tag, this function will not check using strcmp. When the same
// tag/func combination is added multiple times, this function remove exactly
// one candidate, it is undefined which. Returns whether a tag was found (and
// removed).
...
#define dlg_assertm(expr, ...)                                                 \
    dlg_assertlm(DLG_DEFAULT_ASSERT, expr, __VA_ARGS__)
#define dlg_asserttm(tags, expr, ...)                                          \
    dlg_assertltm(DLG_DEFAULT_ASSERT, tags, expr, __VA_ARGS__)
...
//   dlg_assertlm_or(dlg_level_fatal, data != nullptr, return, "Data must not be
//   null"); dlg_assert_or(data != nullptr, logError(); return false);
#define dlg_assertltm_or(level, tags, expr, code, ...)                         \
    dlg__assert_or(level, DLG_CREATE_TAGS tags, expr, code,                    \
                   DLG_FMT_FUNC(__VA_ARGS__))
#define dlg_assertlm_or(level, expr, code, ...)                                \
    dlg__assert_or(level, DLG_CREATE_TAGS(NULL), expr, code,                   \
                   DLG_FMT_FUNC(__VA_ARGS__))
#define dlg_assertl_or(level, expr, code)                                      \
    dlg__assert_or(level, DLG_CREATE_TAGS(NULL), expr, code, NULL)

#define dlg_assert_or(expr, code) dlg_assertl_or(DLG_DEFAULT_ASSERT, expr, code)
#define dlg_assertm_or(expr, code, ...)                                        \
    dlg_assertlm_or(DLG_DEFAULT_ASSERT, expr, code, __VA_ARGS__)
