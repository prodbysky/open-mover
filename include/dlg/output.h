...
// library functions.
DLG_API int dlg_fprintf(FILE *stream, const char *format, ...)
    DLG_PRINTF_ATTRIB(2, 3);
...
// The style will always be applied (using escape sequences), independent of the
// given stream. On windows escape sequences don't work out of the box, see
// dlg_win_init_ansi().
...
// The default level-dependent output styles. The array values represent the
// styles to be used for the associated level (i.e. [0] for trace level).
DLG_API extern const struct dlg_style dlg_default_output_styles[6];

// Generic output function. Used by the default output handler and might be
// useful for custom output handlers (that don't want to manually format the
// output). Will call the given output func with the given data (and format +
// args to print) for everything it has to print in printf format. See also the
// *_stream and *_buf specializations for common usage. The given output
...
// Generic output function. Used by the default output handler and might be
// useful for custom output handlers (that don't want to manually format the
// output). If stream is NULL uses stdout. Automatically uses dlg_fprintf to
// assure correct utf-8 even on windows consoles. Locks the stream (i.e. assures
// threadsafe access) when the associated feature is passed (note that
// stdout/stderr might still mix from multiple threads).
...
// Generic output function (see dlg_generic_output) that uses a buffer instead
// of a stream. buf must at least point to *size bytes. Will set *size to the
// number of bytes written (capped to the given size), if buf == NULL will set
// *size to the needed size. The size parameter must not be NULL.
DLG_API void dlg_generic_output_buf(char *buf, size_t *size,
                                    unsigned int features,
                                    const struct dlg_origin *origin,
                                    const char *string,
                                    const struct dlg_style styles[6]);
DLG_API void dlg_generic_outputf_buf(char *buf, size_t *size,
                                     const char *format_string,
                                     const struct dlg_origin *origin,
                                     const char *string,
                                     const struct dlg_style styles[6]);
...
// Undefined behvaiour if any member of style has a value outside its enum range
// (will probably result in a buffer overflow or garbage being printed). If all
// member of style are 'none' will simply nullterminate the first buf char.
...
// On windows tries to set the console mode to ansi to make escape sequences
// work. This works only on newer windows 10 versions. Returns false on error.
// Only the first call to it will have an effect, following calls just return
// the result. The function is threadsafe. Automatically called by the default
// output handler. This will only be able to set the mode for the stdout and
// stderr consoles, so other streams to consoles will still not work.
